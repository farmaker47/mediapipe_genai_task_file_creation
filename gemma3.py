...
def build_model_1b(
  ...
  """Builds a Gemma3 1B model."""
  # Step 1: Always create the model from a config first.
  # This guarantees `model` is a valid object.
  config = decoder.get_decoder_config_1b()
  model = decoder.Decoder(config, mask_cache_size)

  # Step 2: If a checkpoint is provided, load the weights.
  if checkpoint_path and custom_loader:
    state_dict = custom_loader(checkpoint_path)
